...
def get_brightness() -> int:
    ...
    if dimmed_start_time:
        try:
            ...
            dimmed_start_time = now.replace(hour=dimmed_start_time.hour,
                                            minute=dimmed_start_time.minute,
                                            second=dimmed_start_time.second,
                                            microsecond=0)
        except (ValueError, TypeError):
            click.echo('dimmed_start_time was provided but not in valid ISO 8601 format')
    if dimmed_end_time:
        try:
            dimmed_end_time = datetime.time.fromisoformat(dimmed_end_time)
            dimmed_end_time = now.replace(hour=dimmed_end_time.hour,
